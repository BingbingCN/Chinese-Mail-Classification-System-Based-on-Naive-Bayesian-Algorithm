...
def train():
    global spamDict,normDict,testDict,wordsList,wordsDict,testResult,stopList
    # 保存词频的词典
    spamDict = {}
    normDict = {}
    testDict = {}
...
def single_test():
    ...
    for line in open('temp'):
        ...
        spam.get_word_list(line, wordsList, stopList)
    spam.addToDict(wordsList, wordsDict)
    testDict1 = wordsDict.copy()
    # 通过计算每个文件中p(s|w)来得到对分类影响最大的15个词
    wordProbList = spam.getTestWords(testDict1, spamDict, normDict, normFilelen, spamFilelen)
    print(wordProbList)
    # 对每封邮件得到的15个词计算贝叶斯概率
    p = spam.calBayes(wordProbList, spamDict, normDict)
    if(p>0.9):
